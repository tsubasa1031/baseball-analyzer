...
def main():
    ...
    # B. 選手選択
    st.sidebar.caption("※両方空欄で「リーグ全体」")
    search_mode = st.sidebar.radio("検索方法", ["チームから探す (現役)", "名前検索 (引退/全選手)"], key="step1_search_mode")
    ...
    if search_mode == "チームから探す (現役)":
        ...
        if not roster_df.empty:
            ...
            if p_team != "指定なし":
                ...
                p_select = st.sidebar.selectbox("選手名 (P)", ["指定なし"] + team_ps['Name'].tolist(), key="p_select")
                if p_select != "指定なし":
            ...
            if b_team != "指定なし":
                ...
                b_select = st.sidebar.selectbox("選手名 (B)", ["指定なし"] + team_bs['Name'].tolist(), key="b_select")
                if b_select != "指定なし":
    ...
    if st.session_state.raw_data.empty:
        ...
        if st.sidebar.button("グラフ描画 (Analyze & Plot) 📊", type="secondary"):
            ...
            with col_res1:
                ...
                # 捕手視点の設定 (右がプラス、左がマイナス)
                ax.set_xlim(2.5, -2.5)
                ax.set_ylim(0, 6.0)
                ax.set_aspect('equal')
                ax.set_xlabel("Catcher's View (Left=LHB, Right=RHB)")
                ax.set_title(f"{analysis_type.upper()} Map (Catcher's View)")
                st.pyplot(fig)
